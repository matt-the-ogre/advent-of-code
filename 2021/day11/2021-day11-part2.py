...
def processInput():
    ...
    while allFlashStepNum == -1:
        ...
        if funWithGraphics:
            ...
            plt.savefig(f"{filepath}\images\dumbo{str(stepNum).zfill(4)}.png")
            # plt.show()

        # logging.debug(f"{inputArray[1:inputArray.shape[0]-1, 1:inputArray.shape[1]-1]}")
        logging.debug(f"{stepNum}")
        # check if the 'real' array (not including the padding rows and columns) has every
        # element equal to zero -- using the .all() function here
        # this means everything just flashed
        if numpy.equal(inputArray[1:inputArray.shape[0]-1, 1:inputArray.shape[1]-1], 0).all():
            allFlashStepNum = stepNum + 1
            break
        stepNum += 1

    # logging.debug(f"\n{inputArray}")

    return(allFlashStepNum)
...
def main():
    ...
    if funWithGraphics:
        # I don't know why this isn't working -- after I added the filepath to the output file it stopped
        output = os.system(f"magick convert -delay 5 {filepath}\images\*.png {filepath}\images\anim.mp4")
