...
def processInput(dotArray, foldInstructionList):
    ...
    # for part one we are only doing the first fold, no loop necessary

    # for index, instructionItem in enumerate(foldInstructionList):
    index = 0
    instructionItem = foldInstructionList[index]
    ...
    totalDotsVisible = numpy.sum(dotArray)
    return(totalDotsVisible)
...
def main():
    ...
    numDotsAfterFirstFold = processInput(dotArray, foldInstructionList)

    if unitTesting:
        testPass = False

        # write the assignment of a boolean here that will determine if the unit test passed or not
        testPass = (numDotsAfterFirstFold == 17)

        print("testPass:", testPass)
    else:
        # print the answer here
        print(numDotsAfterFirstFold)
