...
# write a function to check if an MD5 hash has six or more leading zeroes
...
def fiveZeroes(stringToCheck):
    ...
    # check if the first six characters are zeroes
    if md5_value.hexdigest()[:6] == "000000":
        return True
    else:
        return False
...
while fiveZeroes(str(puzzleInput+str(i)).encode('utf-8')) != True:
    ...
    if debug and (i%10000 == 0):
        # if debugging, print every ten-thousandth counter to just see that we're still running!
        print(i)
